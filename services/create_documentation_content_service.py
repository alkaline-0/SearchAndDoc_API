    ...
    async def finalize_document(self, records: list[dict], topic: str) -> list[str]:
        ...
        stream = await self._groq_client.chat.completions.create(
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"""Given the document below, check that it covers all relevant details about {topic} as found in the provided {server_documents} (a list of Discord messages). If you find any important information about {topic} in {server_documents} that is not already in the document, update the technical document to include it. Make sure the final document is accurate and complete regarding {topic}. These versions are concise, clear, and provide explicit instructions for both content creation and validation, while ensuring technical traceability and relevance.""",
                },
            ],
            model=self._groq_config.MODEL,
            temperature=self._groq_config.TEMPERATURE,
        ...
        async for chunk in stream:
            finalized_document.append(chunk.choices[0].delta.content)
