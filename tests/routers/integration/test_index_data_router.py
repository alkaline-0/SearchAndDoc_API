...
class TestIndexDataRouter:
  ...
  async def test_index_data_successfully(self, solr_connection):
    ...
    with TestClient(app) as test_router:
        ...
        with patch.dict(
            ...
            response = test_router.post("/index-data", content=index_data_request.model_dump_json())
  ...
  async def test_index_nonexistent_collection_redirect(self, solr_connection):
    ...
    app = create_app()
    with TestClient(app) as test_router:
        index_data_request = IndexDataRequest(server_id="missing_collection", data=documents)
  ...
  async def test_index_data_internal_error(self, solr_connection):
    ...
    app = create_app()
    with TestClient(app) as test_router:
        create_request = CreatecollectionRequest(server_id="error_collection", shards=1, replicas=1)
        with patch.dict(test_router.app.state.config, {"solr_config": MockSolrConfig()}):
            test_router.post("/create-collection", content=create_request.model_dump_json())

        index_data_request = IndexDataRequest(
            server_id="error_collection",
            data=[{}]
        )
        response = test_router.post("/index-data", content=index_data_request.model_dump_json())
