...
from services.create_collection_service import (CreateCollectionServiceParams,
                                                create_collection)
from services.create_document_service import (CreateDocumentServiceParams,
                                              create_document_service)
from services.index_data_service import (IndexDataServiceParams,
                                         index_data_service)
...
class TestCreateDocumentService:
    @pytest.mark.asyncio
    async def test_successful_document_creation(self, solr_connection: Iterator[ConnectionFactoryService], retriever_model: SentenceTransformerInterface, rerank_model: SentenceTransformerInterface):
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_collection(self, solr_connection: Iterator[ConnectionFactoryService], retriever_model: SentenceTransformerInterface, rerank_model: SentenceTransformerInterface):
    ...
    @pytest.mark.asyncio
    async def test_search_failure(self, solr_connection: Iterator[ConnectionFactoryService], retriever_model: SentenceTransformerInterface, rerank_model: SentenceTransformerInterface):
        ...
        with patch.object(solr_connection, "_logger") as mock_logger:
            ...
            mock_logger.error.assert_called_with(
                'Search failed: not enough values to unpack (expected 1, got 0)', exc_info=True, 
            )
